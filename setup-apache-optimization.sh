...
setup_php_environment() {
    ...
    rm -f "$PHP_ETC/conf.d/magiiic-dev.ini"
    rm -f "$PHP_ETC/conf.d/magiiic-prod.ini"
    
    # Create appropriate symlink
    if [[ "$env" == "production" ]]; then
        ln -sf "$MAGIC_ETC/php/8.3/fpm/conf.d/magiiic-prod.ini" "$PHP_ETC/conf.d/magiiic-prod.ini"
        echo "✓ Enabled production PHP settings"
    else
        ln -sf "$MAGIC_ETC/php/8.3/fpm/conf.d/magiiic-dev.ini" "$PHP_ETC/conf.d/magiiic-dev.ini" 
        echo "✓ Enabled development PHP settings"
    fi
    
    # Ensure defaults are always linked
    ln -sf "$MAGIC_ETC/php/8.3/fpm/conf.d/magiiic-defaults.ini" "$PHP_ETC/conf.d/magiiic-defaults.ini" 2>/dev/null || true
...
main() {
    ...
    echo "  1. Copy your site config to sites-available/ using the new macro format"
