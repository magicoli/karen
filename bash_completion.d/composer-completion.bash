...
COMPOSER_COMPLETION_PHP_SCRIPT=/opt/magic/etc/bash_completion.d/composer-completion.php
...
if [ -z "${COMPOSER_COMPLETION_PHP_SCRIPT}" ] ; then
  check="$(realpath composer-completion.bash | sed "s/\.bash/.php/")"
  [ -f "$check" ] && COMPOSER_COMPLETION_PHP_SCRIPT=$check
fi
